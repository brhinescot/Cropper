...
{
    [TestFixture]
    public class ImageFormatEventArgsTests
    {
        [Test]
        public void ClickedMenuItem_is_null_by_default()
        {
            ImageFormatEventArgs args = new ImageFormatEventArgs();

            Assert.IsNull(args.ClickedMenuItem);
        }

        [Test]
        public void ImageOutputFormat_is_null_by_default()
        {
            ImageFormatEventArgs args = new ImageFormatEventArgs();

            Assert.IsNull(args.ImageOutputFormat);
        }

        [Test]
        public void ClickedMenuItem_property_is_not_altered()
        {
            MenuItem clickedMenuItem = new MenuItem();

            ImageFormatEventArgs args = new ImageFormatEventArgs();
            args.ClickedMenuItem = clickedMenuItem;

            Assert.AreEqual(clickedMenuItem, args.ClickedMenuItem);
        }

        [Test]
        public void ImageOutputFormat_property_is_not_altered()
        {
            IPersistableImageFormat imageOutputFormat = new PersistableImageFormat();

            ImageFormatEventArgs args = new ImageFormatEventArgs();
            args.ImageOutputFormat = imageOutputFormat;

            Assert.AreEqual(imageOutputFormat, args.ImageOutputFormat);
        }

        private class PersistableImageFormat : IPersistableImageFormat
        {
