...
{
    [TestFixture]
    public class ImageCapturedEventArgsTests
    {
        [Test]
        public void FullSizeImage_is_null_by_default()
        {
            ImageCapturedEventArgs args = new ImageCapturedEventArgs();

            Assert.IsNull(args.FullSizeImage);
        }

        [Test]
        public void ThumbnailImage_is_null_by_default()
        {
            ImageCapturedEventArgs args = new ImageCapturedEventArgs();

            Assert.IsNull(args.ThumbnailImage);
        }

        [Test]
        public void IsThumbnailed_is_false_by_default()
        {
            ImageCapturedEventArgs args = new ImageCapturedEventArgs();

            Assert.IsFalse(args.IsThumbnailed);
        }

        [Test]
        public void ImageNames_property_is_not_altered()
        {
            ImagePairNames imageNames = new ImagePairNames("fullSIze", "thumbnail");

            ImageCapturedEventArgs args = new ImageCapturedEventArgs();
            args.ImageNames = imageNames;

            Assert.AreEqual(imageNames, args.ImageNames);
        }

        [Test]
        public void FullSizeImage_property_is_not_altered()
        {
            Image fullSizeImage = new Bitmap(1, 1);

            ImageCapturedEventArgs args = new ImageCapturedEventArgs();
            args.FullSizeImage = fullSizeImage;

            Assert.AreEqual(fullSizeImage, args.FullSizeImage);
        }

        [Test]
        public void ThumbnailImage_property_is_not_altered()
        {
            Image thumbnailImage = new Bitmap(1, 1);

            ImageCapturedEventArgs args = new ImageCapturedEventArgs();
            args.ThumbnailImage = thumbnailImage;

            Assert.AreEqual(thumbnailImage, args.ThumbnailImage);
        }

        [Test]
        public void IsThumbnailed_property_is_not_altered()
        {
            bool isThumbnailed = true;

            ImageCapturedEventArgs args = new ImageCapturedEventArgs();
            args.IsThumbnailed = isThumbnailed;

            Assert.AreEqual(isThumbnailed, args.IsThumbnailed);
        }
    }
}